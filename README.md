...
1. Submit [setup_binaries.sh](setup_binaries.sh) as a batch job, using `sbatch setup_binaries.sh`. This will
   compile the custom version of OpenFAST and ROSCO as a batch job with 8 cores.
2. Run [setup_conda.sh](setup_conda.sh) on the login node. This will setup the
